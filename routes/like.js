...
const LIKE_NOT_FOUND_MESSAGE =
  "You don't liked this post yet. To remove the like first of all like the post";
...
// Add a like
router.post("/like", validateAsync(validatePostId), async (req, res) => {
  try {
...
// Remove the like
router.delete("/like", validateAsync(validatePostId), async (req, res) => {
  try {
    const userId = await getUserId(req.header("token"));
    const { postId } = req.body;
